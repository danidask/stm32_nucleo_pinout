...
def get_arduino_connectors(board) -> list:
    output = []
    board_path = os.path.join(BOARDS_DIR, board)
...
def generate_pinout(board):
    if not os.path.exists(os.path.join(BOARDS_DIR, board)):
    ...
    for connector_file in arduino_connectors:
        ...
        pin_parser.save_json(os.path.join(BOARDS_DIR, board, f"partial_{connector_file}.json"))
    # Morpho part
    morpho_file = os.path.join(BOARDS_DIR, board, "morpho.tsv")
    if not os.path.exists(morpho_file):
        ...
        pin_parser.save_json(os.path.join(BOARDS_DIR, board, "partial_morpho.json"))
...
if __name__ == "__main__":
    ...
    print(f"\nPinout stm32_nucleo_pinout/boards/{args.board}/pinout.json generated successfully")
